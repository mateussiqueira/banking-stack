...
O Banking Challenges utiliza uma estratégia de testes em múltiplas camadas:

1. **Testes Unitários**: Funções e serviços isolados
2. **Testes de Integração**: Interação entre componentes
3. **Testes de API**: Endpoints HTTP/GraphQL
4. **Testes de Transação**: Atomicidade e concorrência
...
### Transaction Tests / Testes de Transação
...
Test results appear in GitHub Actions / Bitbucket Pipelines.
