...
export default async function acquirerRoutes(fastify: FastifyInstance): Promise<void> {
  fastify.post('/iso8583/acquirer/authorize', async (request, reply) => {
    try {
      const format = getFormat(request);
      const contentType = request.headers['content-type'] as string;
      const isoMsg = parseBodyToISOMessage(request.body, contentType);
      const { response, error } = processAuthorization(isoMsg);
      if (error) {
  ...
  fastify.post('/iso8583/acquirer/financial', async (request, reply) => {
    try {
      const format = getFormat(request);
      const contentType = request.headers['content-type'] as string;
      const isoMsg = parseBodyToISOMessage(request.body, contentType);
      const { response, error } = processFinancial(isoMsg);
      if (error) {
        formatResponse(reply, response, format);
        return;
      }
  ...
  fastify.post('/iso8583/acquirer/reversal', async (request, reply) => {
    try {
      const format = getFormat(request);
      const contentType = request.headers['content-type'] as string;
      const isoMsg = parseBodyToISOMessage(request.body, contentType);
      const { response, error } = processReversal(isoMsg);
      if (error) {
        formatResponse(reply, response, format);
        return;
      }
  ...
  fastify.get('/iso8583/acquirer/logs', async (_request, reply) => {
